...
import kr.seok.library.domain.vo.LibraryFileVo;
...
public class FileToTmpStep {
    ...
    public Step fileToTmpStep() {
        return stepBuilderFactory.get(STEP_NAME)
                .<LibraryFileVo, TmpEntity>chunk(10000)
                .reader(fileReader())
    ...
    /* 파일 읽는 Reader */
    private ItemReader<? extends LibraryFileVo> fileReader() {

        return new FlatFileItemReader<LibraryFileVo>() {{
            ...
            setLineMapper(new DefaultLineMapper<LibraryFileVo>() {{
...
//                            setNames(LibraryFileVo.getAllFieldNames());
                            /* 원본 CSV 파일에서 모든 필드가 필요없어서 필요한 컬럼 값만 조회 */
                            setIncludedFields(0, 1, 2, 3);
                        }});
                /* [LineMapper 필수 설정] FieldSet을 Entity와 매핑 설정*/
                setFieldSetMapper(new BeanWrapperFieldSetMapper<LibraryFileVo>() {{
                    /* CSV 파일의 값 부분을 Vo로 매핑 하기 위한 설정 */
                    setTargetType(LibraryFileVo.class);
                }});
            }});
        }};
    }

    /* 파일 데이터를 읽어서 LibraryEntity로 매핑하여 Persistence Context에 저장 */
    private ItemProcessor<? super LibraryFileVo,? extends TmpEntity> fileToTmpProcessor() {
//        return LibraryFileVo::toEntity;
        return null;
    }
